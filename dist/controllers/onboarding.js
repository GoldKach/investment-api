...
export async function submitOnboarding(req, res) {
    try {
        ...
        if (payload.tin) {
            ...
            if (conflict)
                return res.status(409).json({ error: "TIN already in use." });
        }
        // Dates
        ...
        const establishmentDate = parseDate(payload.establishmentDate) ?? new Date("1900-01-01T00:00:00Z"); // REQUIRED in your schema
 ...
 */
export async function validateTin(req, res) {
    try {
