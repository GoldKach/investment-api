...
function parseInclude(q) {
    ...
    const includeUser = set.has("user") || set.has("member") || q.includeUser === "1" || q.includeUser === "true";
    const includePortfolio = set.has("portfolio") || q.includePortfolio === "1" || q.includePortfolio === "true";
    const includeUserAssets = set.has("userassets") || set.has("assets") || q.includeUserAssets === "1" || q.includeUserAssets === "true";
    const include = {};
    if (includeUser) {
...
function pickAllocPercent(pa) {
    return toNumber(pa?.asset?.allocPercent ??
        pa?.asset?.allocationPercentage ??
        pa?.allocPercent ??
        pa?.allocationPercentage, 0);
}
/** compute UPA values from NAV + asset fields */
function computeUPA(nav, allocPercent, costPerShare, closePrice) {
...
async function recomputeUPAsFor(userPortfolioId, client = db) {
...
//   }
// }
export async function createUserPortfolio(req, res) {
    try {
...
--------------------------------------------- */
export async function listUserPortfolios(req, res) {
    try {
...
--------------------------------------------- */
export async function getUserPortfolioById(req, res) {
    try {
        const { id } = req.params;
        if (!id)
...
--------------------------------------------- */
export async function updateUserPortfolio(req, res) {
    try {
        const { id } = req.params;
        if (!id)
            return res.status(400).json({ data: null, error: "Missing id." });
        const { portfolioId, recompute, resetAssets } = req.body;
        ...
        const updated = await db.$transaction(async (tx) => {
            ...
            if (portfolioId && portfolioId !== current.portfolioId) {
                ...
                if (!target)
                    throw new Error("TARGET_PORTFOLIO_NOT_FOUND");
                ...
                if (conflict)
                    throw new Error("DUPLICATE_USER_PORTFOLIO");
...
--------------------------------------------- */
export async function recomputeUserPortfolio(req, res) {
    try {
        const { id } = req.params;
        if (!id)
            return res.status(400).json({ data: null, error: "Missing id." });
...
--------------------------------------------- */
export async function deleteUserPortfolio(req, res) {
    try {
        const { id } = req.params;
        if (!id)
            return res.status(400).json({ data: null, error: "Missing id." });
