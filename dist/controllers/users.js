...
/* Helpers */
const isValidRole = (v) => Object.values(UserRole).includes(v);
const isValidStatus = (v) => Object.values(UserStatus).includes(v);
...
export async function createUser(req, res) {
    const { email, phone, password, firstName, lastName, imageUrl, // optional
    role, // optional
    status, // optional
     } = req.body;
    try {
        ...
        const emailNorm = email.trim().toLowerCase();
        const phoneNorm = phone.trim();
        const roleValue = isValidRole(role) ? role : UserRole.USER;
        const statusValue = isValidStatus(status) ? status : UserStatus.PENDING;
...
export async function updateUser(req, res) {
    const { id } = req.params;
    const { firstName, lastName, email, phone, role, status, password, imageUrl, 
    // ⬇️ add these
    emailVerified, isActive, isApproved, } = req.body;
    try {
        const existingUser = await db.user.findUnique({ where: { id } });
        if (!existingUser)
            return res.status(404).json({ data: null, error: "User not found" });
